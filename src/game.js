...
export function makeObjects(length,seed,course){
 const rng=random(seed),rows=[];let id=0;
 for(let d=90;d<length-45;d+=36+rng()*22){
  const safe=Math.floor(rng()*3)-1;const hazard=rng()<.5?'rock':rng()<.55?'fish':'shark';
  for(let lane=-1;lane<=1;lane++){
   const roll=rng();if(lane===safe){if(roll<.55)rows.push({id:id++,d,lane,x:lane*5,type:roll<.4?'boost':'shield',radius:1.5,height:4});}
   else if(roll<.83){const type=roll<.18?'toxic':hazard;rows.push({id:id++,d,lane,x:lane*5,type,radius:type==='shark'?2:1.65,height:type==='shark'?8:type==='fish'?3.3:2.7,phase:rng()*6.28,move:0});}
  }
 }
 // A separate, clearly signalled crossing; all rows still have an open lane.
 return rows;
}
export function createRace({length=1600,course=COURSES[0],seed=1,laps=3}={}){
 return {phase:'countdown',countdown:3,length,course,laps,distance:0,speed:0,x:0,vx:0,jump:0,vy:0,jumpReady:true,time:0,boost:0,slow:0,shield:0,invulnerable:0,drift:0,wasDrift:false,driftDirection:0,events:[],objects:makeObjects(length,seed,course),consumed:new Set(),pickups:0,hits:0,drifts:0,jumps:0,bestLap:Infinity,lapTimes:[],lapStarted:0,rivals:[{name:'Mako',distance:9,speed:course.speed*.94,lane:-1},{name:'Nori',distance:17,speed:course.speed*.91,lane:1},{name:'Coral',distance:25,speed:course.speed*.88,lane:0}]};
}
export function emit(s,type,text){s.events.push({type,text,time:s.time});if(s.events.length>12)s.events.shift();}
export const objectXAt=(o,time)=>o.x+(o.type==='shark'?Math.sin(time*.85+(o.phase||0))*1.1:o.type==='fish'?Math.sin(time*1.2+(o.phase||0))*.6:0);
export function collect(s,o){
 if(o.type==='boost'){s.boost=Math.max(s.boost,3.2);s.pickups++;emit(s,'boost','부스트! 해류를 타세요');}
...
export function stepRace(s,input,dt){
 ...
 if(s.wasDrift&&!drifting){if(s.drift>=.65){s.boost=Math.max(s.boost,1+s.drift*.85);s.drifts++;emit(s,'drift','드리프트 부스트!');}s.drift=0;}
 s.wasDrift=drifting;
 const target=(input.brake?9:s.course.speed)*(s.boost>0?1.62:1)*(s.slow>0?.49:1)*(drifting?.91:1);
 s.speed+=(target-s.speed)*(1-Math.exp(-dt*(input.brake?4:1.25)));
 const targetV=steer*(drifting?11:14);s.vx+=(targetV-s.vx)*(1-Math.exp(-dt*(drifting?3.5:9)));
 s.x+=s.vx*dt;if(Math.abs(s.x)>7.2){s.x=clamp(s.x,-7.2,7.2);s.vx*=.35;s.speed=Math.max(8,s.speed-dt*8);}
 ...
 for(const o of s.objects){
  // Check every object crossed by the swept movement, including the lap seam.
  for(let lap=previousLap;lap<=Math.floor(s.distance/s.length);lap++){
